...
>
> **Load this skill BEFORE any console/UAM *write*, not just reads.** Add-note, set-analyst-verdict, set-status, assign, and IOC-create all run through the UAM `alertTriggerActions` mutation with a specific action-id + `OrFilterSelectionInput` shape (see `references/UNIFIED_ALERTS.md`): action ids are `S1/alert/addNote` / `analystVerdictUpdate` / `statusUpdate`, and the alert is targeted by `filter:{or:[{and:[{fieldId:"id",stringEqual:{value:<id>}}]}]}`, the alert id is the FILTER, never the action `id`. Hand-rolling this GraphQL from live schema introspection is the fast path to a wrong action-id/filter and a misleading empty result. Also: `alertTriggerActions` returns a success-shaped `ActionsTriggered` even when nothing changed, so pass the payload through `action_outcome()` (it reduces the union to `applied` plus the per-alert reasons) **and** re-query (`uam_get_alert`) to confirm the write landed. The refusal reason is in `actions[].failure[].errorMessage`, which is why `trigger_actions` selects that field. **Do not treat that string as a diagnosis.** `Missing UAM manage permissions` is also what a *not-offered* action returns: alerts ingested through the UAM Alert Interface (`/v1/alerts`) offer only `addNote` and `eventSearch`, while the same token applies `statusUpdate` to a native alert successfully. Before stating why an action failed, call `available_actions(...)` with a narrow filter and read `isDisabled` / `disabledReason`; `trigger_actions` does this for you on failure (`diagnose_failures=True`) and `explain_action_failure` reports `not_offered` / `disabled` / `offered`. Availability is also scope-sensitive: the incident actions are `INCIDENT_ACTIONS_ONLY_AVAILABLE_FROM_SITE_VIEW` under ACCOUNT scope and enabled under SITE. And a `not_offered` result is not proof that no route exists: search in this order, **UAM availability, then the Hyperautomation integration catalog (`hyperautomation/references/integration-catalog.md` lists native `Set Alert Status to Resolved` / `Resolve Alert as …` / `Verdict …` actions with their `public_action_id`), then the console UI**, and only then report that nothing can do it.
